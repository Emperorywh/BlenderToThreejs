    ...
    for lower in (False, True):
        for j in range(nr + 1):
            ...
            for u, v in perimeter:
                z = p["eave_z"] + p["rise"] * (1 - t) ** hall_config["roof_curve_power"]
                z += p["corner_lift"] * t ** 6 * abs(u * v) ** 5 + p["edge_lift"] * t ** 8
                z += .22 * abs(u) ** 8 * (1 - t) ** 8
                verts.append((center_xy[0] + hx * u, center_xy[1] + hy * v, z - (hall_config["roof_thickness"] if lower else 0)))
    count = n * (nr + 1)
    faces = []
