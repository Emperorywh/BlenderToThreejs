...
def route_geometry(route, config):
    ...
    profile.extend([(length, b.z - route["thickness"]), (0, a.z - route["thickness"])])
    verts = []
    for side in (-1, 1):
    ...
    return {"id": route["id"], "horizontal_length": length, "rise": b.z - a.z, "slope_degrees": math.degrees(math.atan2(b.z - a.z, length)), "step_rise": rise, "step_run": tread}
...
def setup_render(scene, config):
    ...
    scene.render.filepath = str(OUT / "previews" / "overview.png")
...
def main():
    ...
    target = OUT / "tiangong_whitebox.blend"
    ...
    scene["supplemental_design"] = "背面、底部收分、不可见连接和首版尺寸均为补充设计"
